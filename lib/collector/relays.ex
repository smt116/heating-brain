defmodule Collector.Relays do
  ...
  def put_state(%RelayState{id: id, value: value}) do
    if relay_value(id) == value do
      ...
      |> get_env(:heating_brain, :filesystem_handler).write!(raw_value)
  ...
  @spec setup_all :: :ok
  def setup_all, do: get_env(:heating_brain, :relays_map) |> Enum.each(&setup/1)
  ...
  defp relay_direction(id) do
    ...
    |> get_env(:heating_brain, :filesystem_handler).read!()
    |> String.trim()
  end
  ...
  defp relay_directory_path(id) do
    {_id, pin, _direction, _} =
      get_env(:heating_brain, :relays_map) |> Enum.find(&(elem(&1, 0) === id))

    Path.join([get_env(:heating_brain, :gpio_base_path), "gpio#{to_string(pin)}"])
  end
  ...
  defp setup({id, pin, direction, _load}) do
    handler = get_env(:heating_brain, :filesystem_handler)
    ...
    if relay_directory_path(id) |> handler.dir?() do
      ...
      :ok =
        get_env(:heating_brain, :gpio_base_path)
        |> Path.join("export")
